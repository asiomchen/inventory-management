...
from data import db, Product, InvoiceProduct, Invoice
...
dashboard = Blueprint('analytics', __name__, 
...
def best_selling_products():
    ...
    query = query.with_entities(Product.idx, Product.title, func.sum(InvoiceProduct.quantity).label('total_quantity'))
    ...
    fig = px.bar(bestsellers, x='Total Quantity', y='Title', 
                 title=f'Top {lim} Best Selling Products', hover_data=['Total Quantity'])
...
def get_profit():
    query = Invoice.query
    query = query.with_entities(Invoice.date, func.sum(Invoice.total_customer_price).label('total_profit'))
